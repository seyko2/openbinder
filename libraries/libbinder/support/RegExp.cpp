...
const char *kMeta = "^$.[()|?+*\\";
const int32_t kMaxSize = 32767L;		// Probably could be 65535L.
...
SRegExp::SRegExp(const char *pattern)
	:	fError(B_OK),
		fRegExp(NULL)
{
...
SRegExp::SRegExp(const SString &pattern)
	:	fError(B_OK),
		fRegExp(NULL)
{
...
status_t
SRegExp::InitCheck() const
{
	return fError;
}

status_t
SRegExp::SetTo(const char *pattern)
{
	fError = B_OK;
	free(fRegExp);
	fRegExp = Compile(pattern);
...
status_t
SRegExp::SetTo(const SString &pattern)
{
	fError = B_OK;
	free(fRegExp);
	fRegExp = Compile(pattern.String());
...
bool
SRegExp::Matches(const char *string) const
{
	if (!fRegExp || !string)
		return false;
...
bool
SRegExp::Matches(const SString &string) const
{
	if (!fRegExp)
		return false;
...
regexp *
SRegExp::Compile(const char *exp)
{
	regexp *r;
	const char *scan;
	const char *longest;
	...
	if (*Next((char *)scan) == kRegExpEnd) {		// Only one top-level choice.
		...
		if (flags&kSPStart) {
			...
			for (; scan != NULL; scan = Next((char *)scan))
				if (*scan == kRegExpExactly && (int32_t)strlen(Operand(scan)) >= len) {
...
regexp *
SRegExp::Expression() const
{
	return fRegExp;
}

const char *
SRegExp::ErrorString() const
{
...
void
SRegExp::SetError(status_t error) const
{
	fError = error;
}
...
//
char *
SRegExp::Reg(int32_t paren, int32_t *flagp)
{
	char *ret;
	char *br;
	char *ender;
	...
	*flagp = kHasWidth;	// Tentatively.
	...
	if (paren) {
		if (fParenthesisCount >= kSubExpressionMax) {
			SetError(B_REGEXP_TOO_MANY_PARENTHESIS);
			return NULL;
		}
	...
	*flagp |= flags&kSPStart;
	...
	if (paren && *fInputScanPointer++ != ')') {
		SetError(B_REGEXP_UNMATCHED_PARENTHESIS);
		return NULL;
...
SRegExp::Piece(int32_t *flagp)
{
	...
	if (!(flags & kHasWidth) && op != '?') {
		SetError(B_REGEXP_STAR_PLUS_OPERAND_EMPTY);
		return NULL;
	}
...
SRegExp::Atom(int32_t *flagp)
{
	...
	*flagp = kWorst;		// Tentatively.
	...
	switch (*fInputScanPointer++) {
		...
		case '[':
			{
					...
					if (*fInputScanPointer == '-') {
						...
						if (*fInputScanPointer == ']' || *fInputScanPointer == '\0')
							...
							if (cclass > classend + 1) {
								SetError(B_REGEXP_INVALID_BRACKET_RANGE);
								return NULL;
							}
							for (; cclass <= classend; cclass++)
								EmitChar((char)cclass);
							fInputScanPointer++;
						}
					} else
						EmitChar(*fInputScanPointer++);
				...
				EmitChar('\0');
				if (*fInputScanPointer != ']') {
					SetError(B_REGEXP_UNMATCHED_BRACKET);
					return NULL;
				}
				fInputScanPointer++;
				*flagp |= kHasWidth | kSimple;
		...
		case '|':
		case ')':
			SetError(B_REGEXP_INTERNAL_ERROR);
			return NULL; //  Supposed to be caught earlier.
		case '?':
		case '+':
		case '*':
			SetError(B_REGEXP_QUESTION_PLUS_STAR_FOLLOWS_NOTHING);
			return NULL;
		case '\\':
			if (*fInputScanPointer == '\0') {
				SetError(B_REGEXP_TRAILING_BACKSLASH);
				return NULL;
			}
...
//
void
SRegExp::OpTail(char *p, char *val)
{
...
//
int32_t
SRegExp::RunMatcher(regexp *prog, const char *string) const
{
	const char *s;
...
///
int32_t			// 0 failure, 1 success
SRegExp::Match(const char *prog) const
{
	...
	while (scan != NULL) {
		...
		switch (*scan) {
			...
			default:
				SetError(B_REGEXP_MEMORY_CORRUPTION);
				return 0;
			}
	...
	//
	SetError(B_REGEXP_CORRUPTED_POINTERS);
	return 0;
}
...
//
int32_t
SRegExp::Repeat(const char *p) const
{
	int32_t count = 0;
	const char *scan;
	const char *opnd;
...
const char *
SRegExp::Next(const char *p) const
{
	int32_t offset;
...
inline int32_t
SRegExp::UCharAt(const char *p) const
{
	return (int32_t)*(unsigned char *)p;
}

inline char *
SRegExp::Operand(char* p) const
{
	return p + 3;
}

inline const char *
SRegExp::Operand(const char* p) const
{
	return p + 3;
}

inline bool
SRegExp::IsMult(char c) const
{
	return c == '*' || c == '+' || c == '?';
}
...
//
char *
SRegExp::Prop(const char *op) const
{
	char *p = NULL;
	static char buf[50];
...
void
SRegExp::RegExpError(const char *) const
{
